...
# Новые 2 метода
    def create_pet_simple(self, auth_key: json, name: str, animal_type: str, age: int) -> json:
        """Метод отправляет запрос на сервер для добавления информации о новом питомце без фото
    ...
    def add_pet_photo(self, auth_key: json, pet_id: str, pet_photo: str) -> json:
        """Метод отправляет запрос на сервер для добавления фотографии питомца по указанному ID и
        возвращает статус запроса и результат в формате JSON с данными обновленного питомца"""
